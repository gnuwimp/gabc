...
//------------------------------------------------------------------------------
class Parameters(val source: String = "", val dest: String = "", val cover: String = "", val author: String = "", val title: String = "", val year: String = "", val comment: String = "", val genre: String = "", val bitrate: String = "", val gap: String = "", val mono: Boolean = false, val vbr: Boolean = false) {
    var audioFiles: List<File> = listOf()
    val mp3                  = if (year != "") File("$dest${File.separator}$author - $title ($year).mp3") else File("$dest${File.separator}$author - $title.mp3")
    ...
    fun validate() {
        when {
            ...
            author.isBlank() -> throw Exception("error: artist/author string is empty")
